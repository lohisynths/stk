...
inline StkFloat BowTable :: tick( StkFloat input )
{
  ...
  lastFrame_[0] = (StkFloat) math::fabs( (double) sample ) + (StkFloat) 0.75;
  lastFrame_[0] = (StkFloat) math::pow( lastFrame_[0], (StkFloat) -4.0 );
...
inline StkFrames& BowTable :: tick( StkFrames& frames, unsigned int channel )
{
  ...
  for ( unsigned int i=0; i<frames.frames(); i++, samples += hop ) {
    ...
    *samples *= slope_;
    *samples = (StkFloat) math::fabs( (double) *samples ) + 0.75;
    *samples = (StkFloat) math::pow( *samples, (StkFloat) -4.0 );
...
inline StkFrames& BowTable :: tick( StkFrames& iFrames, StkFrames& oFrames, unsigned int iChannel, unsigned int oChannel )
{
  ...
  for ( unsigned int i=0; i<iFrames.frames(); i++, iSamples += iHop, oSamples += oHop ) {
    ...
    *oSamples *= slope_;
    *oSamples = (StkFloat) math::fabs( (double) *oSamples ) + 0.75;
    *oSamples = (StkFloat) math::pow( *oSamples, (StkFloat) -4.0 );
