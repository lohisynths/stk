...
{
  filter.a[1] = -2.0 * radius * math::cos( TWO_PI * frequency / Stk::sampleRate());
...
inline void Shakers :: waterDrop( void )
{
  ...
  for ( unsigned int i=0; i<3; i++ ) { // WATER_RESONANCES = 3
    ...
    if ( filters_[i].gain > 0.001 ) {
      ...
      filters_[i].a[1] = -2.0 * baseRadii_[i] * math::cos( TWO_PI * tempFrequencies_[i] / Stk::sampleRate() );
    }
    else
      filters_[i].gain = 0.0;
  }
      ...
      if ( randomFloat( 1024.0 ) < nObjects_ ) {
        ...
        for ( unsigned int i=0; i<nResonances_; i++ ) {
          if ( doVaryFrequency_[i] ) {
            ...
            filters_[i].a[1] = -2.0 * baseRadii_[i] * math::cos( TWO_PI * tempRand / Stk::sampleRate() );
          }
        }
