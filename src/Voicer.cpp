...
{
  unsigned int i;
  StkFloat frequency = (StkFloat) 220.0 * math::pow( 2.0, (noteNumber - 57.0) / 12.0 );
...
void Voicer :: pitchBend( StkFloat value, int group )
{
  ...
  if ( value < 8192.0 )
    pitchScaler = math::pow( 0.5, (8192.0-value) / 8192.0 );
  else
    pitchScaler = math::pow( 2.0, (value-8192.0) / 8192.0 );
  for ( unsigned int i=0; i<voices_.size(); i++ ) {
    if ( voices_[i].group == group )
