...
Asymp :: Asymp( void )
{
  ...
  target_ = 0.0;
  state_ = 0;
  factor_ = math::exp( -1.0 / ( 0.3 * Stk::sampleRate() ) );
  constant_ = 0.0;
...
void Asymp :: sampleRateChanged( StkFloat newRate, StkFloat oldRate )
{
  if ( !ignoreSampleRateChange_ ) {
    StkFloat tau = -1.0 / ( math::log( factor_ ) * oldRate );
    factor_ = math::exp( -1.0 / ( tau * newRate ) );
  }
}
...
void Asymp :: setTau( StkFloat tau )
{
  ...
  factor_ = math::exp( -1.0 / ( tau * Stk::sampleRate() ) );
...
void Asymp :: setTime( StkFloat time )
{
  ...
  StkFloat tau = -time / math::log( TARGET_THRESHOLD );
  factor_ = math::exp( -1.0 / ( tau * Stk::sampleRate() ) );
  constant_ = ( 1.0 - factor_ ) * target_;
}
