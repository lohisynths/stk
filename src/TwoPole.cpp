...
void TwoPole :: setResonance( StkFloat frequency, StkFloat radius, bool normalize )
{
  ...
  a_[2] = radius * radius;
  a_[1] = (StkFloat) -2.0 * radius * math::cos(TWO_PI * frequency / Stk::sampleRate());

  if ( normalize ) {
    // Normalize the filter gain ... not terribly efficient.
    StkFloat real = 1 - radius + (a_[2] - radius) * math::cos(TWO_PI * 2 * frequency / Stk::sampleRate());
    StkFloat imag = (a_[2] - radius) * math::sin(TWO_PI * 2 * frequency / Stk::sampleRate());
    b_[0] = math::sqrt( math::pow(real, 2) + math::pow(imag, 2) );
  }
}
